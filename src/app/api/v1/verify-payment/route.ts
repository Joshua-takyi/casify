...
export async function POST(req: Request) {
    try {
        ...
        if (!secretKey) {
            ...
            return NextResponse.json({status: 'error', message: 'Missing secret key'}, {status: 500});
        }
        ...
        if (data.status === true && data.data.status === 'success') {
            return NextResponse.json({status: 'success', message: 'Payment verified'}, {status: 200});
        } else {
